...
from BioCycleModel.Multimodal_ClosedLoopModel import ClosedLoopModel
...
def main(model_config = None):
    ...
    while True:
        ...
        valid_loss, valid_results, valid_truths, predict_results_val, pred_score_val, pred_score_s2_val = valid(dataloader["val"], trainer, criterion, modelConfig)
        valid_acc, valid_f1, valid_pre, valid_rec = eval_FakeSV(valid_results, valid_truths)
        # test for one epoch
        test_loss, test_results, test_truths, predict_results_test, pred_score_test, pred_score_s2_test = valid(dataloader["test"], trainer, criterion, modelConfig)
        test_acc, test_f1, test_pre, test_rec = eval_FakeSV(test_results, test_truths)
        save_path = f"predict_results/predict_results_test_epoch{epoch}.json"
        score_save_path = f"pred_scores/pred_score_test_epoch{epoch}.txt"
        score_save_s2_path = f"pred_scores_s2/pred_score_s2_test_epoch{epoch}.txt"
        with open(save_path, "w", encoding="utf-8") as f:
            for record in predict_results_test:
                f.write(json.dumps(record, ensure_ascii=False) + "\n")
        with open(score_save_path, "w", encoding="utf-8") as f1:
            for s in pred_score_test:
                f1.write(" ".join(map(str, s)) + "\n")
        with open(score_save_s2_path, "w", encoding="utf-8") as f1:
                    for s in pred_score_s2_test:
                        f1.write(" ".join(map(str, s)) + "\n")

        loss_config.append(train_loss)
        ...
        if modelConfig["dataset"] in ['mosi', 'mosei_senti', 'WEIBO']:
            ...
            print('|| Train F1 {:5.4f} | Val f1 {:5.4f} | Test f1 {:5.4f}'.format(train_f1, valid_f1, test_f1))
            ...
            if test_acc >= (best_test_acc + 1e-4):
                print('{} dataset | acc improved! saving model to {}_best_test_model.pkl'
                      .format(modelConfig["dataset"], modelConfig["dataset"]))
                torch.save(trainer, '{}_best_test_model.pkl'.format(modelConfig["dataset"]))

                best_test_acc = test_acc
                best_f1_test = test_f1
                best_pre_test = test_pre
                best_rec_test = test_rec
    ...
    if modelConfig["dataset"] in ['mosi', 'mosei_senti', 'WEIBO']:
        ...
        print('-' * 50)
        print('--------------------best test output: ↓↓↓↓↓↓↓↓↓---------------------------')
        print("Best Epoch:", best_epoch_test)
        print("Best Acc: {:5.4f}".format(best_test_acc))
        print("f1: {:5.4f}".format(best_f1_test))
        print("pre: {:5.4f}".format(best_pre_test))
        print("rec: {:5.4f}".format(best_rec_test))
        print('-' * 50)
        print(loss_config)
