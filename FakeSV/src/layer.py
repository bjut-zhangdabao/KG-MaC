...
class TuckERLayer(nn.Module):
    def __init__(self, dim, r_dim, output_dim=None):
        super(TuckERLayer, self).__init__()
        self.dim = dim
        self.r_dim = r_dim   # 384
        self.output_dim = output_dim

        self.W = nn.Parameter(torch.empty(r_dim, dim, dim))
        nn.init.xavier_uniform_(self.W.data) # 384 128 128

        self.relation_embed = nn.Parameter(torch.randn(1, r_dim))
        ...
        self.out_drop = nn.Dropout(0.4)

        self.out_proj = nn.Linear(dim, self.output_dim)

    def forward(self, x1, x2):
        """
        x1: (B, len1, dim)  - 主序列（保留这个序列）
        x2: (B, len2, dim)  - context（用于融合）
        return: (len1, B, output_dim)
        """
        B, len1, dim = x1.shape
        _, len2, _ = x2.shape
        assert dim == self.dim
        # Normalize & dropout
        x1 = self.bn1(x1.reshape(-1, dim)).reshape(B, len1, dim)
        x1 = self.drop1(x1)
        x2 = self.bn2(x2.reshape(-1, dim)).reshape(B, len2, dim)
        x2 = self.drop2(x2)
        # relation embedding
        r_embed = self.relation_embed.expand(B, -1)  # (B, r_dim) (16,384)
        W_flat = self.W.view(self.r_dim, -1)         # (r_dim, dim*dim) (384, 128*128)
        rW = torch.matmul(r_embed, W_flat)           # (B, dim*dim)
        W_r = rW.view(B, dim, dim)                   # (B, dim, dim)
        # einsum 融合，得到 (B, len1, len2)
        scores = torch.einsum('bld,bdh,bmd->blm', x1, W_r, x2)
        # 计算 x1 每个位置的加权融合表示
        attn = torch.softmax(scores, dim=2)  # (B, len1, len2)
        fused_x2 = torch.einsum('blm,bmd->bld', attn, x2)  # (B, len1, dim)
        fused = x1 + fused_x2  # (B, len1, dim)
        fused = self.out_proj(fused.view(-1, dim)).view(B, len1, self.output_dim)
        fused = self.out_bn(fused.view(-1, self.output_dim)).view(B, len1, self.output_dim)
        fused = self.out_drop(fused)
        # 转换为 (len1, B, output_dim)
        fused = fused.permute(1, 0, 2)
        return fused  # shape: (len1, B, output_dim)
