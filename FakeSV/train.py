...
def train(modelConfig: Dict, train_loader, trainer, criterion, optimizer):
    ...
    for batch in tqdm(train_loader):
        ...
        loss_output = (torch.abs(bce_loss_output - 0.1) + 0.1) + (torch.abs(bce_loss_output_ - 0.1) + 0.1)*0.01 + diffusion_loss*1.0
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            for v, p, l, s, s2, c in zip(vid, y.cpu().tolist(), labels.cpu().tolist(), pred.cpu().tolist(), pred_.cpu().tolist(), confidence_score.cpu().tolist()):
                record = {
                    "video_id": v,
                    "predict_result": "真" if p == 0 else "假",
                    "annotation": "真" if l == 0 else "假",
                    "real_score": f"{s[0]:.4f}",
                    "fake_score": f"{s[1]:.4f}",
